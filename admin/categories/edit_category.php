...
if ($category_id) {
    ...
    $error = "ID Kategori tidak diberikan.";
}

// Proses form jika ada data POST
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $new_category_name = trim($_POST['category_name'] ?? '');
    $post_category_id = $_POST['category_id'] ?? null; // ID dari hidden input

    if (empty($new_category_name)) {
        $error = "Nama kategori tidak boleh kosong.";
    } elseif (!$post_category_id || (int)$post_category_id !== (int)$category_id) {
        $error = "ID Kategori tidak valid.";
    } else {
        // Cek apakah nama kategori sudah ada (selain kategori yang sedang diedit)
        $stmt_check = $conn->prepare("SELECT id FROM categories WHERE name = ? AND id != ?");
        $stmt_check->bind_param("si", $new_category_name, $post_category_id);
        $stmt_check->execute();
        ...
        if ($result_check->num_rows > 0) {
            ...
            // Lakukan update data
            $stmt_update = $conn->prepare("UPDATE categories SET name = ?, updated_at = NOW() WHERE id = ?");
            $stmt_update->bind_param("si", $new_category_name, $post_category_id);
            if ($stmt_update->execute()) {
                $message = "Kategori berhasil diperbarui!";
                $category_name = $new_category_name; // Perbarui nama yang ditampilkan
            } else {
...
<head>
    ...
    <meta name="description" content="Edit Kategori Produk SkinGlow!">
    <meta name="author" content="Tim SkinGlow">
    <title>SkinGlow! - Edit Kategori</title>
        ...
        <div id="content-wrapper" class="d-flex flex-column">
            <div id="content">
                ...
                <div class="container-fluid">
                    ...
                    <?php if ($category_id && empty($error)): // Tampilkan form hanya jika ID kategori valid dan tidak ada error fatal ?>
                        <div class="card shadow mb-4">
                            ...
                            <div class="card-body">
                                <form action="edit_category.php?id=<?php echo htmlspecialchars($category_id); ?>" method="POST">
                                    ...
                                    <a href="manage_categories.php" class="btn btn-secondary"><i class="fas fa-arrow-left"></i> Kembali</a>
                                </form>
                            </div>
                        </div>
                    <?php else: ?>
                        <a href="manage_categories.php" class="btn btn-primary">Kembali ke Daftar Kategori</a>
                    <?php endif; ?>
